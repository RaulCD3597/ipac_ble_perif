...
#define DEVICE_NAME                     "IPAC_perif"                              /**< Name of device. Will be included in the advertising data. */
    ...
    switch (pin_no)
    {
        case EMERGENCY_BUTTON:
            if (APP_BUTTON_PUSH == button_action)
            {
                do
                {
                    ...
                    err_code = ble_nus_data_send(&m_nus, data_array[0], &length, m_conn_handle);
        ...
        case SERVICE_BUTTON:
            if (APP_BUTTON_PUSH == button_action)
            {
                ...
                {
                    uint16_t length = (uint16_t)9;
                    err_code = ble_nus_data_send(&m_nus, data_array[1], &length, m_conn_handle);
                    if ((err_code != NRF_ERROR_INVALID_STATE) &&
                        (err_code != NRF_ERROR_RESOURCES) && (err_code != NRF_ERROR_NOT_FOUND))
                    {
                        APP_ERROR_CHECK(err_code);
